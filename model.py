...
class Tripletnet(nn.Module):
    ...
    def forward(self, x, y, z, c, img11=None, img111=None):
        """ x: Anchor image,
        ...
        if img11 is None:
            ...
            embedded_x11 = self.embeddingnet(img11, c)
            embedded_x111 = self.embeddingnet(img111, c)
            # embedded_x1111 = self.embeddingnet(img1111, c)

            sim_a11 = torch.sum(embedded_x11 * embedded_y, dim=1)
            sim_b11 = torch.sum(embedded_x11 * embedded_z, dim=1)
            ...
            sim_a11111 = torch.sum(embedded_x * embedded_y, dim=1)
            sim_b11111 = torch.sum(embedded_x * embedded_x111, dim=1)

            return sim_a, sim_b, sim_a11, sim_b11, sim_a111, sim_b111, sim_a1111, sim_b1111, sim_a11111, sim_b11111
